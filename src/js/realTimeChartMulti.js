...
function realTimeChartMulti() {
  ...
  var version = "0.1.0",
    datum, data,
    maxSeconds = 1000, pixelsPerSecond = 10,
    svgWidth = 700, svgHeight = 300,
    margin = { top: 20, bottom: 30, left: 100, right: 30, topNav: 20, bottomNav: 20 },
    dimension = { chartTitle: 20, xAxis: 20, yAxis: 10, xTitle: 20, yTitle: 20, navChart: 70 },
  ...
  var chart = function (s) {
    ...
    svg = selection.append("svg")
      ...
      .style("border", function (d) {
        if (border) return "2px solid #CEDBDB";
        else return null;
      });
    ...
    y = d3v3.scale.ordinal().domain(yDomain).rangeRoundPoints([height, 0], 1)

    // define main chart axis
    xAxis = d3v3.svg.axis().orient("bottom");
    yAxis = d3v3.svg.axis().orient("left");
    ...
    xNav = d3v3.time.scale().range([0, widthNav]);
    yNav = d3v3.scale.ordinal().domain(yDomain).rangeRoundPoints([heightNav, 0], 1)

    // define nav axis
    var xAxisNav = d3v3.svg.axis().orient("bottom");

    // compute initial time domains...
    var ts = 1534395958000; //start time
    ...
    function getTagName(that) {
      var tagName = d3v3.select(that).node().tagName;
      return (tagName);
    }
